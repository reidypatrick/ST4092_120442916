draw_network <- function(name,
  ...
  network_tex <- c(
    ...
    "every node/.style={scale=0.8}]
            % Input layer
            \\node[circle, draw, fill=blue!20] (input1) at (",
    margin, ",", v_scale * (-1), ") {$x_1$};
            \\node[circle, draw, fill=blue!20] (input2) at (",
    margin, ",", v_scale * (-2), ") {$x_2$};
            \\node[circle, draw, fill=blue!20] (input3) at (",
    margin, ",", v_scale * (-3), ") {$\\ldots$};
            \\node[circle, draw, fill=blue!20] (input4) at (",
    margin, ",", v_scale * (-4), ") {$x_{", n_input, "}$};

            % First hidden layer
            \\path (", margin + h_scale * 2, ",", v_scale * (-1),
    ") node[circle, draw, fill=green!20] (hidden11) {$h_{1,1}$};
            \\path (", margin + h_scale * 2, ",", v_scale * (-2),
    ") node[circle, draw, fill=green!20] (hidden12) {$h_{1,2}$};
            \\path (", margin + h_scale * 2, ",", v_scale * (-3),
    ") node[circle, draw, fill=green!20] (hidden13) {$\\ldots$};
            \\path (", margin + h_scale * 2, ",", v_scale * (-4),
    ") node[circle, draw, fill=green!20] (hidden14) {$h_{1, {",
    n_hidden1, "}}$};

            % Second hidden layer
            \\path (", margin + h_scale * 4, ",", v_scale * (-1),
    ") node[circle, draw, fill=green!20] (hidden21) {$h_{2,1}$};
            \\path (", margin + h_scale * 4, ",", v_scale * (-2),
    ") node[circle, draw, fill=green!20] (hidden22) {$h_{2,1}$};
            \\path (", margin + h_scale * 4, ",", v_scale * (-3),
    ") node[circle, draw, fill=green!20] (hidden23) {$\\ldots$};
            \\path (", margin + h_scale * 4, ",", v_scale * (-4),
    ") node[circle, draw, fill=green!20] (hidden24) {$h_{2, {",
    n_hidden2, "}}$};

            % Output layer
            \\path (", margin + h_scale * 6, ",", v_scale * (-2.5),
    ") node[circle, draw, fill=red!20] (output) {$y$};
