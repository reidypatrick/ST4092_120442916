...
*What data set is being used, Discuss each plot in turn and insights yielded, Motivation behind each plot* This study is carried out using the French motor third partly liabilty claims frequency data (FreMTPL2freq) available through CASdatasets [@CASdatasets].

```{r Table1, echo=FALSE, include=TRUE, paged.print=TRUE}

head(cars_orig) %>% 
kable(format = "latex", booktabs = TRUE, caption = "Head of the Cars Dataset", row.names = FALSE) %>%
