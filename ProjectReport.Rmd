...
## Exponential Dispersion Family {.unnumbered}
...
\end{figure}
```
Above can be seen two representations of the architecture underlying NNs. Figure \ref{fig: figIntroNetwork } shows the fully connected network, with two hidden layers. Figure \ref{fig:figIntroDummyCompGraph} shows the computation graph where:
...
It will eventually be seen how we arrive at this model, but firstly, we consider a NN at its simplest. We define a NN with 0 hidden layers, and some activation function $f$. Visualised in a similar fashion to \@ref(fig:figIntroDummyCompGraph) we get
